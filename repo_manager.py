...
def main(arg_1 = None):
    ...
    # Update plugins description with supported aries-cloudagent version
        ...
        # If the version matches the global version, then it was not upgraded
        plugins_on_old_release = []
        for plugin in last_releases:
            name = plugin.split('|')[1].strip()
            if name in all_plugins: 
                all_plugins.remove(name)
            if (plugin.split('|')[2].strip() == global_version[0]):
                plugins_on_old_release.append(name)

        # Add any new plugins. Considered upgraded.
        merged_plugins = list(plugins_on_old_release)
        merged_plugins.extend(plugin for plugin in all_plugins if plugin not in merged_plugins)
        
        [print(plugin) for plugin in merged_plugins]
